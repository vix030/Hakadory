...
function init() {
  ...
  resetAlert();
  refresh();
  /* もう使わない鍵を片付ける（タスク連携をやめる前の版が残したもの）。
   * 実績は最大 2000 件ぶん貯まるので、置いたままだと保存領域を食う。
   * しばらく経って行き渡ったら、この 3 行は消してよい。 */
  for (const key of ['Hakadory.link', 'Hakadory.laps']) {
    try { localStorage.removeItem(key); } catch (error) { /* 消せなくても困らない */ }
  }
